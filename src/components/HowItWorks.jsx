      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        <FadeIn>
          <div className="text-center max-w-3xl mx-auto mb-20">
            <h2 className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white border border-primary/20 text-primary text-[11px] font-bold tracking-widest uppercase mb-6 shadow-sm">
              How It Works
            </h2>
            <h3 className="text-4xl md:text-5xl font-extrabold mb-6 tracking-tight text-slate-900">Up and running in under 10 minutes</h3>
            <p className="text-lg md:text-xl text-slate-600 leading-relaxed">
              No developers. No code. No complicated configuration. Syncify is built for business teams, not IT departments.
            </p>
          </div>
        </FadeIn>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-8 relative z-10">
            {[
              ...
              <FadeIn key={idx} delay={0.1 * idx} className="flex h-full">
                <div className="bg-green-100/40 backdrop-blur-lg rounded-2xl p-8 border border-white/30 shadow-lg hover:shadow-xl hover:-translate-y-1.5 transition-all w-full flex flex-col items-center text-center group">
                  <div className="w-16 h-16 bg-white border-2 border-primary/20 text-primary rounded-full flex items-center justify-center text-2xl font-extrabold mb-6 shadow-sm relative group-hover:scale-105 transition-transform">
                    <item.icon className="w-6 h-6 absolute opacity-10" />
                    <span className="relative z-10">{item.step}</span>
                  </div>
                  <h4 className="text-2xl font-bold mb-3 text-slate-900 tracking-tight">{item.title}</h4>
        ...
        <FadeIn delay={0.5}>
          <div className="mt-20 bg-gradient-to-r from-lightGreen/50 to-white rounded-[2.5rem] p-10 flex flex-col sm:flex-row items-center justify-between border border-primary/20 shadow-xl relative overflow-hidden">
            <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] opacity-5"></div>
            <div className="mb-8 sm:mb-0 text-center sm:text-left relative z-10">
              <h4 className="text-2xl font-extrabold mb-2 text-slate-900 tracking-tight">See Syncify in action</h4>
              <p className="text-slate-600 font-medium">Live, with your questions answered</p>
            </div>
